...
1. 安装 Microsoft Print to PDF 打印机，运行 optionalfeatures 或在 控制面版-程序-启用或关闭 Windows 功能 中勾选 Microsoft Print to PDF 选项。
...
本质是：Outlook 应该通过 autodiscover.ourdomain.com 与内部 Exchange 服务器通信。而 Outlook 首先认为是 O365 User。

需要添加注册表表项：

`HKEY_CURRENT_USER\Software\Microsoft\Office\16.0\Outlook\AutoDiscover` 中 新建 `ExcludeExplicitO365Endpoint` 表项，类型为 DWORD，值为 1。适用于 2019 及以上版本。

或者可以尝试，在 hosts 文件中增加 `127.0.0.1 Outlook.office365.com`

值得注意的是，完全正常的 Outlook 是不会有 AutoDiscover 这个表项的。
