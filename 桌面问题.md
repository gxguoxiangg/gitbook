...
##	Q4	Microsoft Print to PDF

使用虚拟打印机 *Microsoft Print to PDF* 可用来导出 PDF 版格式，一般情况下系统自带，如果没有请按照如下操作重新安装：
...
处理这类问题时，那么需要注意处理的顺序，通过**逐层排查**，快速定位故障层级，避免做无用功：

**物理层：检查物理连接、地址信息 (IPv4，DNS，网关等)：**
...
1. ping 检测网关的通信情况，检查延迟。如果同一网线在另一台主机可以上网，且在无法上网的主机上进入 PE 依旧无法正常上网。则极大概率是封禁了 MAC，或者是网关设备地址分配配置问题。

**三层：ping 不同网段的主机，如DNS等，检查域名解析情况：**
...
##	Q69	Win11 底部任务栏消失，按Win键开始菜单不弹出
...
##	Q70	Word 正文中，行的末尾空了一个字符

类似这样：![img](https://iknow-pic.cdn.bcebos.com/4610b912c8fcc3ce545edf4c8245d688d43f2036?x-bce-process=image%2Fresize%2Cm_lfit%2Cw_600%2Ch_800%2Climit_1%2Fquality%2Cq_85%2Fformat%2Cf_auto)
...
##	Q71	MacOS 下 Outlook 提示此时无法接收邮件

MacOS下弹出提示框：账号“xxxx@xxxx”的服务器返回了错误"登录失败:未知用户名或密码错误"。您的用户名/密码或安全设置可能不正确。是否希望尝试重新输入密码。
...
3. 按下Ctrl + J，将图层复制出来。
4. 右键该图层，点击`混合选项`，选择`颜色叠加`，选择颜色为黑色。此时抠出的签名在颜色和形状上已经完整。
5. 导出该图层为PNG格式。



##	Q74	关闭休眠，删除hiberfil.sys节省空间
...
##	Q78	Edge配置企业站点模式，使得网站默认以指定的IEX(7 ~ 11)文档模式打开

####	下载添加 Edge 策略文件

下载企业版Edge的策略文件：

![EdgeForBusiness](https://img2024.cnblogs.com/blog/2470142/202406/2470142-20240619103812897-1507087852.png)

下载地址：https://www.microsoft.com/en-us/edge/business/download

解压后，将`MicrosoftEdgePolicyTemplates\windows\admx`路径下的如下文件复制粘贴到`C:\Windows\PolicyDefinitions`目录下：
...
- `配置Internet Explorer集成`：已启用，Internet Explorer模式
- `允许访问Enterprise Mode Site List Manager工貝`：已启用

####	配置企业站点列表

在 edge 中输入 `edge://compat`，选择企业网站列表管理器，点击添加站点：

![image4](https://img2024.cnblogs.com/blog/2470142/202406/2470142-20240619103952931-532722569.png)

随后导出XML到指定位置。

####	启动企业站点列表

在组策略管理器中，打开 `计算机配置->模板管理->Microsoft Edge`，选择 `配置企业模式站点列表`，启用后配置企业模式站点列表，格式如下：

`file:///C:/xx/xx/xx.xml`



##	Q79	BIOS 中关于 CPU C-State 设置引起的死机现象

C-States（C状态）为软件提供了请求CPU进入低功耗状态的能力，通过关闭core或其他逻辑单元来实现的。

C-States 从 C0开始，C后面的数字越大，表示CPU进入休眠模式的程度越深。开启C-States时，可能会出现电脑待机时死机、蓝屏的现象。
...
Excel中，选中对有勾选筛选的区域复制时，会提示无法对合并单元格执行此操作。

问题原因：

- 首部几行，存在合并情况，并且筛选应用于前面几行。

解决方案：

- 取消筛选，将筛选应用于首行的合并单元格的最末尾的那一行。
